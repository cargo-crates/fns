...
pub fn throttle<F, T>(closure: F, delay: Duration) -> Throttle<T> where F: Fn(T) -> () + Send + Sync + 'static, T: Send + Sync + 'static {
          ...
          match message {
            Ok(param) => {
              if let Some(param) = param {
                ...
                if closure_time.is_none() || now.duration_since(closure_time.unwrap()) >= throttle_config.delay {
                  current_param = None;
...
impl<T> Drop for ThrottleConfig<T> {
  fn drop(&mut self) {
    log::trace!("drop ThrottleConfig {:?}", format!("{:p}", self));
  }
}
...
impl<T> Drop for Throttle<T> {
  fn drop(&mut self) {
    self.terminate();
    log::trace!("drop Throttle {:?}", format!("{:p}", self));
  }
}
...
mod tests {
  ...
  fn it_works() {
    ...
    let throttle_fn = throttle(move |param| {
